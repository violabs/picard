...
data class PodFailurePolicyOnExitCodesRequirement(
     ...
     *     is in the set of specified values.
     * NotIn: the requirement is satisfied if at least one container exit code (might be multiple if there
     *       are multiple containers not restricted by the 'containerName' field) is not in the set of specified
     *      values. Additional values are considered to be added in the future. Clients should react to an unknown
     *      operator by assuming the requirement is not satisfied.
     */
    val operator: Operator,
    /**
     * Specifies the set of values. Each returned container exit code
     * (might be multiple in case of multiple containers) is checked against this set
     * of values with respect to the operator. The list of values must be ordered
