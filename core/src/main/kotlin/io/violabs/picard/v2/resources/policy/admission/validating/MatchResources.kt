...
data class MatchResources(
     ...
     * ValidatingAdmissionPolicy should not care about. The exclude rules take precedence 
     ...
     *
     * Exact: match a request only if it exactly matches a specified rule. For example, 
     * if deployments can be modified via apps/v1, apps/v1beta1, and extensions/v1beta1, 
     * but "rules" only included apiGroups:["apps"], apiVersions:["v1"], resources: ["deployments"], 
     * a request to apps/v1beta1 or extensions/v1beta1 would not be sent to the ValidatingAdmissionPolicy.
     *
     * Equivalent: match a request if modifies a resource listed in rules, even via another 
     * API group or version. For example, if deployments can be modified via apps/v1, apps/v1beta1, 
     ...
     * converted to apps/v1 and sent to the ValidatingAdmissionPolicy.
     *
     * Defaults to "Equivalent"
     */
    val matchPolicy: String? = null,
    /**
     * NamespaceSelector decides whether to run the admission control policy on an object 
     ...
     * object is another cluster scoped resource, it never skips the policy.
     *
     ...
     * "namespaceSelector": { "matchExpressions": [ { "key": "environment", "operator": "In", 
     * "values": [ "prod", "staging" ] } ] }
     *
     ...
     * delete) or an object that cannot have labels (like a DeploymentRollback or a PodProxyOptions 
     * object) is not considered to match. Use the object selector only if the webhook is opt-in, 
     * because end users may skip the admission webhook by setting the labels. Default to the 
     ...
     * ValidatingAdmissionPolicy matches. The policy cares about an operation if it matches any Rule.
     */
