...
data class CsiDriverSpec(
     ...
     * and that the Kubernetes attach detach controller should call the attach volume
     * interface which checks the volumeattachment status and waits until the volume
     * is attached before proceeding to mounting. The CSI external-attacher coordinates
     ...
     * value is specified to false, the attach operation will be skipped. Otherwise
     * the attach operation will be called.
     *
     * This field is immutable.
     */
    val attachRequired: Boolean? = null,
    /**
     ...
     */
    val fsGroupPolicy: String? = null,
    /**
     * nodeAllocatableUpdatePeriodSeconds specifies the interval between periodic updates
     * of the CSINode allocatable capacity for this driver. When set, both periodic
     * updates and updates triggered by capacity-related failures are enabled. If not
     * set, no updates occur (neither periodic nor upon detecting capacity-related failures),
     * and the allocatable.count remains static. The minimum allowed value for this field
     * is 10 seconds.
     *
     * This is an alpha feature and requires the MutableCSINodeAllocatableCount feature
     * gate to be enabled.
     *
     * This field is mutable.
     */
    val nodeAllocatableUpdatePeriodSeconds: Long? = null,
    /**
     * podInfoOnMount indicates this CSI volume driver requires additional pod information
     * (like podName, podUID, etc.) during mount operations, if set to true. If set to false,
     * pod information will not be passed on mount. Default is false.
     *
     * The CSI driver specifies podInfoOnMount as part of driver deployment. If true,
     * Kubelet will pass pod information as VolumeContext in the CSI NodePublishVolume()
     * calls. The CSI driver is responsible for parsing and validating the information
     * passed in as VolumeContext.
     *
     * The following VolumeContext will be passed if podInfoOnMount is set to true. This
     * list might grow, but the prefix will be used. "csi.storage.k8s.io/pod.name":
     * pod.Name "csi.storage.k8s.io/pod.namespace":
     * pod.Namespace "csi.storage.k8s.io/pod.uid":
     * string(pod.UID) "csi.storage.k8s.io/ephemeral":
     * "true" if the volume is an ephemeral inline volume defined
     * by a CSIVolumeSource, otherwise "false"
     *
     * "csi.storage.k8s.io/ephemeral" is a new feature in Kubernetes 1.16.
     * It is only required for drivers which support both the "Persistent" and "Ephemeral"
     * VolumeLifecycleMode. Other drivers can leave pod info disabled and/or ignore this
     * field. As Kubernetes 1.15 doesn't support this field, drivers can only support
     * one mode when deployed on such a cluster and the deployment determines which
     * mode that is, for example via a command line parameter of the driver.
     *
     * This field was immutable in Kubernetes < 1.29 and now is mutable.
     */
    val podInfoOnMount: Boolean? = null,
    /**
     ...
     * NodePublishVolume should only update the contents of the volume. New mount points
     * will not be seen by a running container.
     */
    val requiresRepublish: Boolean? = null,
    /**
     ...
     * devices or as independent shared volumes. Kubernetes will call NodeStage / NodePublish
     * with "-o context=xyz" mount option when mounting a ReadWriteOncePod volume used in
     * Pod that has explicitly set SELinux context. In the future, it may be expanded to
     * other volume AccessModes. In any case, Kubernetes will ensure that the volume is
     ...
     */
    val storageCapacity: Boolean? = null,
    /**
     ...
     */
    val tokenRequests: List<TokenRequest>? = null,
    /**
     ...
     *
     * The other mode is "Ephemeral". In this mode, volumes are defined inline inside the
     * pod spec with CSIVolumeSource and their lifecycle is tied to the lifecycle of that
     * pod. A driver has to be aware of this because it is only going to get a
     * NodePublishVolume call for such a volume.
     *
     * For more information about implementing this mode,
     * see https://kubernetes-csi.github.io/docs/ephemeral-local-volumes.html A driver
     * can support one or more of these modes and more modes may be added in the future.
     *
     * This field is beta. This field is immutable.
     */
    val volumeLifecycleModes: List<String>? = null
) : BaseSpec