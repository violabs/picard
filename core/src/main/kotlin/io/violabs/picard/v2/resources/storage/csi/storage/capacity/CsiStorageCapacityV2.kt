 ...
 *
 * For example this can express things like: - StorageClass "standard" has "1234 GiB" available
 * in "topology.kubernetes.io/zone=us-east1" - StorageClass "localssd" has "10 GiB" available
 * in "kubernetes.io/hostname=knode-abc123"
 *
 * The following three cases all imply that no capacity is available for a certain
 * combination: - no object exists with suitable topology and storage class name - such an
 * object exists, but the capacity is unset - such an object exists, but the capacity is zero
 *
 ...
 * requested size of pending volumes to filter out unsuitable nodes. If MaximumVolumeSize is unset,
...
data class CsiStorageCapacityV2(
    ...
    /**
     * Standard object's metadata. The name has no particular meaning. It must be a
     * DNS subdomain (dots allowed, 253 characters). To ensure that there are no conflicts
     * with other CSI drivers on the cluster, the recommendation is to use csisc-<uuid>,
     * a generated name, or a reverse-domain name which ends with the unique CSI driver name.
     *
     ...
     * capacity applies to. It must meet the same requirements as the name of a StorageClass
     * object (non-empty, DNS subdomain). If that object no longer exists, the CSIStorageCapacity
     ...
     *
     * The semantic is currently (CSI spec 1.2) defined as: The available capacity, in bytes, of
     * the storage that can be used to provision volumes. If not set, that information is
     ...
     * If not set, the storage is not accessible from any node in the cluster. If empty, the storage is
     * accessible from all nodes. This field is immutable.
     */
    val nodeTopology: LabelSelector? = null
) : ConfigResource<CsiStorageCapacityV2.Version, ObjectMeta> {
