...
data class JobSpec(
     ...
     * than this number when ((.spec.completions - .status.successful) < .spec.parallelism),
     ...
     * limited to 1 and the success of that pod signals the success of the job.
     * More info: https://kubernetes.io/docs/concepts/workloads/controllers/jobs-run-to-completion/
     */
     ...
     * active Pods associated with this Job. Users must design their workload to gracefully handle this.
     ...
     * user is responsible for picking unique labels and specifying the selector. Failure to
     * pick a unique label may cause this and other jobs to not function correctly. However, You
     * may see manualSelector=true in jobs that were created with the old extensions/v1beta1 API.
     ...
     * of actions and conditions which need to be satisfied to take the associated action. If empty,
    ...
    /**
     * successPolicy specifies the policy when the Job can be declared as succeeded. If empty, the
     * default behavior applies - the Job is declared as succeeded only when the number of succeeded
     * pods equals to the completions. When the field is specified, it must be immutable and works only
     * for the Indexed Jobs. Once the Job meets the SuccessPolicy, the lingering pods are terminated.
     */
    val successPolicy: SuccessPolicy? = null,
    /**
     ...
     * set when Job's completionMode=Indexed, and the Pod's restart policy is Never. The field is immutable.
     */
     ...
     * characters trailing the first "/" must be valid HTTP Path characters as defined by RFC 3986.
     * The value cannot exceed 63 characters. This field is immutable.
     */
     ...
     * left as null the job continues execution of all of its indexes and is marked with
     * the Complete Job condition. It can only be specified when backoffLimitPerIndex is
     * set. It can be null or up to completions. It is required and must be less than or
     * equal to 10^4 when is completions greater than 10^5.
     */
    val maxFailedIndexes: Int? = null,
    /**
     * podReplacementPolicy specifies when to create replacement Pods.
     * Possible values are: - TerminatingOrFailed means that we recreate pods when they
     * are terminating (has a metadata.deletionTimestamp) or failed. Failed means to wait
     * until a previously created Pod is fully terminated (has phase Failed or Succeeded)
     * before creating a replacement Pod. When using podFailurePolicy, Failed is the the only
     * allowed value. TerminatingOrFailed and Failed are allowed values when podFailurePolicy
     * is not in use. This is an beta field. To use this, enable the JobPodReplacementPolicy
