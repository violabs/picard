...
data class ParamRef(
     ...
     * when this field is left unset. Take care that if this is left empty the binding must not match any 
     * cluster-scoped resources, which will result in an error.
     */
    val namespace: String? = null,
    /**
     * parameterNotFoundAction controls the behavior of the binding when the resource exists, and name or selector 
     * is valid, but there are no parameters matched by the binding. If the value is set to Allow, then no matched 
     * parameters will be treated as successful validation by the binding. If set to Deny, then no matched parameters 
     * will be subject to the failurePolicy of the policy.
     *
    ...
    /**
     * selector can be used to match multiple param objects based on their labels. Supply selector: {} to match 
     * all resources of the ParamKind.
     *
