 ...
 * 
 * DeviceSubRequest is similar to ExactDeviceRequest, but doesn't expose the AdminAccess field as that one 
 * is only supported when requesting a specific device.
 */
@GeneratedDsl(withListGroup = true)
data class DeviceSubRequest(
    /**
     * DeviceClassName references a specific DeviceClass, which can define additional configuration 
     * and selectors to be inherited by this subrequest.
     * 
     ...
     * configurations for the claim. References must use the format <main request>/<subrequest>.
     * 
     * Must be a DNS label.
     */
    val name: String,
     ...
     * 
     * ExactCount: This request is for a specific number of devices. This is the default. 
     * The exact number is provided in the count field.
     * 
     * All: This subrequest is for all of the matching devices in a pool. Allocation will fail 
     * if some devices are already allocated, unless adminAccess is requested.
     * 
     * If AllocationMode is not specified, the default mode is ExactCount. If the mode is ExactCount 
     * and count is not specified, the default count is one. Any other subrequests must specify this field.
     * 
    ...
    val count: Long? = null,
    /**
     * Selectors define criteria which must be satisfied by a specific device in order for that device 
     * to be considered for this subrequest. All selectors must be satisfied for a device to be considered.
     * 
     ...
     * 
     * In addition, should any of the allocated devices get tainted with NoExecute after allocation 
     * and that effect is not tolerated, then all pods consuming the ResourceClaim get deleted to evict them. 
