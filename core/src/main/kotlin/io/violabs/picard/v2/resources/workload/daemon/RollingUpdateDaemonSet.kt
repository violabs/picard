...
data class RollingUpdateDaemonSet(
    /**
     * The maximum number of nodes with an existing available DaemonSet
     ...
     * calculated from percentage by rounding up to a minimum of 1.
     * Default value is 0.
     ...
     * their a new pod created before the old pod is marked as deleted. The update starts
     * by launching new pods on 30% of nodes. Once an updated pod is available
     * (Ready for at least minReadySeconds) the old DaemonSet pod on that node is marked deleted.
     * If the old pod becomes unavailable for any reason (Ready transitions to false, is
     * evicted, or is drained) an updated pod is immediatedly created on that node without
     * considering surge limits. Allowing surge implies the possibility that the resources consumed
     * by the daemonset on any given node can double if the readiness check fails, and so resource
     * intensive daemonsets should take into account that they may cause evictions during disruption.
     */
    val maxSurge: IntOrString? = null,
    /**
     ...
     * percentage by rounding up. This cannot be 0 if MaxSurge is 0 Default value is 1.
     * Example: when this is set to 30%, at most 30% of the total number of nodes that
     * should be running the daemon pod (i.e. status.desiredNumberScheduled) can have
