...
data class TopologySpreadConstraint(
     ...
     * MinDomains. For example, in a 3-zone cluster, MaxSkew is set to 1, and
     * pods with the same labelSelector spread as 2/2/1: In this case, the global
     ...
     * of pods into each bucket. We define a domain as a particular instance of a
     * topology. Also, we define an eligible domain as a domain whose nodes meet
     * the requirements of nodeAffinityPolicy and nodeTaintsPolicy.
     *
     ...
     * in any location, but giving higher precedence to topologies that would
     * help reduce the skew. A constraint is considered "Unsatisfiable" for an
     * incoming pod if and only if every possible node assignment for that pod
     * would violate "MaxSkew" on some topology. For example, in a 3-zone cluster,
     * MaxSkew is set to 1, and pods with the same labelSelector spread
     * as 3/1/1: | zone1 | zone2 | zone3 | | P P P | P | P | If WhenUnsatisfiable
     ...
     * which spreading will be calculated. The keys are used to lookup
     * values from the incoming pod labels, those key-value labels are
     * ANDed with labelSelector to select the group of existing pods over
     * which spreading will be calculated for the incoming pod. The same
     * key is forbidden to exist in both MatchLabelKeys and LabelSelector.
     * MatchLabelKeys cannot be set when LabelSelector isn't set. Keys that
     * don't exist in the incoming pod labels will be ignored. A null or
     ...
     * less than minDomains, Pod Topology Spread treats "global minimum"
     ...
     * minDomains, scheduler won't schedule more than maxSkew Pods to
     * those domains. If value is nil, the constraint behaves as if
     * MinDomains is equal to 1. Valid values are integers greater than
     * 0. When value is not nil, WhenUnsatisfiable must be DoNotSchedule.
     *
     * For example, in a 3-zone cluster, MaxSkew is set to 2, MinDomains
     * is set to 5 and pods with the same labelSelector spread as
     * 2/2/2: | zone1 | zone2 | zone3 | | P P | P P | P P | The number
     * of domains is less than 5(MinDomains), so "global minimum" is
     * treated as 0. In this situation, new pod with the same labelSelector
     * cannot be scheduled, because computed skew will be 3(3 - 0) if new
     ...
     * with tainted nodes for which the incoming pod has a toleration, are included.
