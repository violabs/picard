...
data class StatefulSetSpec(
     ...
     * will fulfill this Template, but have a unique identity from the rest of the StatefulSet.
     * Each pod will be named with the format <statefulsetname>-<podindex>. For example, a pod in a
     ...
     */
    val updateStrategy: StatefulSetUpdateStrategy? = null,
    /**
     ...
     * pods in parallel to match the desired scale without waiting, and on scale down will delete all pods at once.
     */
    val podManagementPolicy: String? = null,
    /**
     * revisionHistoryLimit is the maximum number of revisions that will be maintained in
     * the StatefulSet's revision history. The revision history consists of all revisions
     * not represented by a currently applied StatefulSetSpec version. The default value is 10.
     */
    val revisionHistoryLimit: Int? = null,
    /**
     * volumeClaimTemplates is a list of claims that pods are allowed to reference. The
     * StatefulSet controller is responsible for mapping network identities to claims in
     * a way that maintains the identity of a pod. Every claim in this list must have at
    ...
    /**
     * persistentVolumeClaimRetentionPolicy describes the lifecycle of persistent volume
     * claims created from volumeClaimTemplates. By default, all persistent volume claims
     * are created as needed and retained until manually deleted. This policy allows the
     * lifecycle to be altered, for example by deleting persistent volume claims when their
     * stateful set is deleted, or when their pod is scaled down.
     */
    val persistentVolumeClaimRetentionPolicy: StatefulSetPersistentVolumeClaimRetentionPolicy? = null,
    /**
     * ordinals controls the numbering of replica indices in a StatefulSet. The default
     * ordinals behavior assigns a "0" index to the first replica and increments the index
     * by one for each additional replica requested.
     */
    val ordinals: StatefulSetOrdinals? = null
)