...
data class NodeConfigStatus(
     ...
     * LastKnownGood config, depending on whether attempting to use the Assigned config results in an error.
     */
    val active: NodeConfigSource? = null,
    /**
     * Assigned reports the checkpointed config the node will try to use. When
     * Node.Spec.ConfigSource is updated, the node checkpoints the associated config payload
     * to local disk, along with a record indicating intended config. The node refers to this
     ...
     * points while syncing config. Earlier errors (e.g. download or checkpointing errors) will not
     * result in a rollback to LastKnownGood, and may resolve across Kubelet retries. Later errors
     * (e.g. loading or validating a checkpointed config) will result in a rollback to LastKnownGood.
     * In the latter case, it is usually possible to resolve the error by fixing the config assigned
     * in Spec.ConfigSource. You can find additional information for debugging by searching the error
     * message in the Kubelet log. Error is a human-readable description of the error state; machines
     * can check whether or not Error is empty, but should not rely on the stability of the Error text
     * across Kubelet versions.
     */
    val error: String? = null,
    /**
     * LastKnownGood reports the checkpointed config the node will fall back to when it encounters an
     * error attempting to use the Assigned config. The Assigned config becomes the LastKnownGood
     * config when the node determines that the Assigned config is stable and correct. This is
     * currently implemented as a 10-minute soak period starting when the local record of Assigned
